...
(async function init(){
  ...
  if(isEnrolled){
    ...
    enrollBtn.onclick = async ()=>{
      ...
      try {
        const res = await fetch('http://skilltobill-backend.onrender.com:5000/api/orders/course', {
          method: 'POST',
          headers: {
        ...
        const options = {
          ...
          handler: async function (response) {
            // Verify payment
            const verifyRes = await fetch('http://skilltobill-backend.onrender.com:5000/api/orders/verify', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
